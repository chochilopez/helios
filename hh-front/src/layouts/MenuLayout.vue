    ...
    <q-header bordered class="cp12-bc2 text-white fuente1">
      <q-toolbar>
        ...
        <q-btn-dropdown
          ...
          <q-item clickable v-close-popup @click="closeSession">
          <q-item-section>
...
export default {
  setup () {
    ...
    const username = ref(autenticacionService.getUsername())
    const router = useRouter()

    const closeSession = () => {
      autenticacionService.fDoLogout()
    ...
    useMeta(() => {
      return {
        ...
        meta: {
          googlebot: { name: 'googlebot', content: 'noindex, nofollow' }
        }
      }
    })
