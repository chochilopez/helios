    ...
    axios.get(API_URL + 'cheque/buscar-todas', {
      ...
      .then((result) => {
        if (result.status === 200) {
          llaveroService.guardarEnLocalConSesion('hhChequeTodasConSesion', result.data, sesion)
        }
        resolve(result)
      })
      .catch((error) => {
...
function spfBuscarTodasConEliminadasConSesion (sesion) {
  return new Promise((resolve, reject) => {
    axios.get(API_URL + 'cheque/buscar-todas-con-eliminadas', {
      ...
      .then((result) => {
        if (result.status === 200) {
          llaveroService.guardarEnLocalConSesion('hhChequeTodasConEliminadasConSesion', result.data, sesion)
        }
        resolve(result)
      })
      .catch((error) => {
...
function spfReciclar (id) {
  return new Promise((resolve, reject) => {
    axios.get(API_URL + 'cheque/reciclar/' + id, {
      headers: {
        Authorization: 'Bearer ' + autenticacionService.obtenerToken()
      }
    })
