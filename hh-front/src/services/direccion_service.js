...
function spfBuscarTodasConSesion (sesion) {
  return new Promise((resolve, reject) => {
    axios.get(API_URL + 'direccion/buscar-todas', {
      ...
      .then((result) => {
        if (result.status === 200) {
          llaveroService.guardarEnLocalConSesion('hhDireccionTodasConSesion', result.data, sesion)
        }
        resolve(result)
      })
      .catch((error) => {
...
function spfBuscarTodasConEliminadasConSesion (sesion) {
  return new Promise((resolve, reject) => {
    axios.get(API_URL + 'direccion/buscar-todas-con-eliminadas', {
      ...
      .then((result) => {
        if (result.status === 200) {
          llaveroService.guardarEnLocalConSesion('hhDireccionTodasConEliminadasConSesion', result.data, sesion)
        }
        resolve(result)
      })
      .catch((error) => {
...
function spfReciclar (id) {
  return new Promise((resolve, reject) => {
    axios.get(API_URL + 'direccion/reciclar/' + id, {
      headers: {
        Authorization: 'Bearer ' + autenticacionService.obtenerToken()
      }
    })
