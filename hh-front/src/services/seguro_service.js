    ...
    axios.get(API_URL + 'seguro/buscar-todas', {
      ...
      .then((result) => {
        if (result.status === 200) {
          llaveroService.guardarEnLocalConSesion('hhSeguroTodasConSesion', result.data, sesion)
        }
        resolve(result)
      })
      .catch((error) => {
...
function spfBuscarTodasConEliminadasConSesion (sesion) {
  return new Promise((resolve, reject) => {
    axios.get(API_URL + 'seguro/buscar-todas-con-eliminadas', {
      ...
      .then((result) => {
        if (result.status === 200) {
          llaveroService.guardarEnLocalConSesion('hhSeguroTodasConEliminadasConSesion', result.data, sesion)
        }
        resolve(result)
      })
      .catch((error) => {
...
function spfReciclar (id) {
  return new Promise((resolve, reject) => {
    axios.get(API_URL + 'seguro/reciclar/' + id, {
      headers: {
        Authorization: 'Bearer ' + autenticacionService.obtenerToken()
      }
    })
